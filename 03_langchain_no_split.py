...
# MAGIC %pip install -U transformers langchain chromadb pypdf pycryptodome accelerate https://github.com/explosion/spacy-models/releases/download/en_core_web_sm-3.5.0/en_core_web_sm-3.5.0.tar.gz
...
review_texts = spark.sql("""select recommendationid, timestamp_created, left(review, 1000) as review from steam_appreviews_bronze where appid = '1063730' and len(review) > 500""")
...
documents = [Document(page_content=r["review"], metadata={"source": r["recommendationid"]}) for r in review_texts.collect()]
...
db_persist_path_chroma = f"{cloud_storage_path}langchain/nosplit/chroma/db" 
